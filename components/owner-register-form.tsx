...
export function OwnerRegisterForm() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (payload.signedIn) {
        ...
        setMessage(payload.message || "Akun dibuat. Periksa email untuk konfirmasi.")
      }
    } catch (err) {
  ...
  return (
    <form className="surface owner-form" onSubmit={submit}>
      ...
      <input id="owner-name" value={fullName} onChange={(e) => setFullName(e.target.value)} minLength={2} required />
      ...
      <input id="owner-register-password" type="password" autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={8} required />
      <p className="form-note">Minimal 8 karakter. Akun pemilik langsung aktif setelah pendaftaran berhasil.</p>
